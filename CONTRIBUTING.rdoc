...
So you want to contribute to ruby-dicom? That's great, thanks! If you're a
first time committer, you may want to take a second to read these instructions.
...
* Fork the repository (for bonus points, create a topical branch name).
* Execute the specification (tests) to verify that all spec examples pass
...
* Wait for feedback (this shouldn't take too long). The pull request may be accepted right
  away, it may be rejected (with a reason specified), or it may be discussed with
  changes/improvements suggested in order for it to be accepted.
...
* No trailling whitespace. Blank lines should not have any space.
...
Copyright assignment is the preferred and encouraged option for larger
code contributions, and will be assumed unless otherwise is specified.
...
in the README file. If such an accreditation is not wanted, please let me know so,
either in the pull request or in private.


= Other contributions

Don't want to get your hands dirty with source code and git? Don't worry,
there are other ways you can contribute to the project as well!

* Create an issue on github for feature requests or bug reports.
* Write a tutorial.
* Answer questions, or tell others about your ruby-dicom projects in the
