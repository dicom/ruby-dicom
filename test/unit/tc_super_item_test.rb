...
class TC_SuperItemTest < Test::Unit::TestCase
  ...
  def test_decode_pixels
    ...
    assert_raise(RuntimeError) {@obj2.decode_pixels(nil)} # Should raise error as the argument must be a String.
  ...
  def test_encode_pixels
    ...
    assert_raise(RuntimeError) {@obj2.encode_pixels(nil)} # Should raise error as the argument must be an Array.
