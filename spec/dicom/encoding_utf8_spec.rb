...
module DICOM
  ...
  describe DObject do
    ...
    describe "#write" do
      ...
      it "should properly write the ASCII-8BIT and UTF-8 string values to the DICOM file" do
        ...
        @dcm.write(TMPDIR + 'utf8/utf8.dcm')
        reloaded_dcm = DObject.read(TMPDIR + 'utf8/utf8.dcm')
  ...
  describe Element do
    ...
    before :example do
      @dcm = DObject.new
      Element.new('0008,0005', 'ISO_IR 100', :parent => @dcm)
    end
  ...
  describe Anonymizer do
    ...
    before :example do
      @a = Anonymizer.new(:audit_trail => TMPDIR + 'audit_trail_utf8.json')
      @a.write_path = TMPDIR + 'anon/utf8/'
      @a.enumeration = true
      @a.set_tag("0008,1090", :value => "Manufacturer", :enum => true)
      @a.anonymize_path(TMPDIR + 'utf8/')
    end
    ...
    describe "#value=" do
      ...
      it "should create an audit file containing the expected utf8 and ascii values" do
        at = AuditTrail.new
        at.load(TMPDIR + 'audit_trail_utf8.json')
